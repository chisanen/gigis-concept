...
export function InstagramFeed({ widgetId, handle }: Props) {
  ...
  // --- Fallback: linked grid to their Instagram profile ---
  if (handle) {
    return (
      <div className="w-full">
          ...
          {Array.from({ length: 8 }).map((_, i) => (
            ...
            >
              <div className="text-center">
                <svg
                  className="w-8 h-8 mx-auto text-brand-400 mb-2"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  aria-hidden="true"
                >
                ...
                <p className="text-[10px] text-brand-500 tracking-[0.1em]">
                  @{handle}
                </p>
              </div>
  ...
  return (
    <div className="text-center py-12">
      <p className="text-[13px] text-brand-500">
        Connect your Instagram in Admin &rarr; Site Settings &rarr; Social to
        show your feed here. For an auto-updating feed, paste your free Elfsight
        widget ID.
      </p>
    </div>
  );
